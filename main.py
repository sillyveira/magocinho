...
from config import *
from sprites import Block, Chao, Item, Barra, Moeda
...
class TelaInicial:
    def __init__(self):
        pygame.init()
        pygame.mixer.init()
        self.tela = pygame.display.set_mode((LARGURA_TELA, ALTURA_TELA), pygame.FULLSCREEN)
...
class JOGO:
    ...
    def new(self):
        ...
        self.inimigo_tipo1 = Inimigo(self, 5, 10, 1)
        self.inimigo_tipo2 = Inimigo(self, 5, 4, 2)
    ...
    def atualizar_itens(self):
        ...
        while len(espacos) != 0:
            random_x_y = random.choice(espacos)
            x, y = random_x_y[0], random_x_y[1]
    ...
    def draw(self): # Desenhando os sprites na tela
        ...
        self.tela.blit(self.player.imagex, (self.player.rect.x - 5, self.player.rect.y - 20))   
        self.tela.blit(texto, texto_formatado)
        if self.player.intangivel == True:
            self.tela.blit(self.barreira, (self.player.rect.x-5, self.player.rect.y-20))
